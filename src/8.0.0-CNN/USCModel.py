...
class USCModel :
 ...
 def prepareData(self,data,augment):
  ...
  ###x_data=self.uscData.normalize(x_data).reshape(x_data.shape[0],x_data.shape[1],1)
...
###  return encodedValue,y_data_one_hot_encoded
 ...
 def train(self,data):
  ...
  self.model.fit(x_data, y_data,validation_data=(x_data,y_data), batch_size = self.uscData.mini_batch_size)
...
###   layer_input = keras.layers.Input(batch_shape=(self.uscData.mini_batch_size,1,self.uscData.track_length))
   out=keras.layers.Convolution1D(64, 3,activation='relu', border_mode='same')(layer_input) # out.shape=(self.uscData.mini_batch_size,1,16)
   out=keras.layers.Convolution1D(64, 3,activation='relu', border_mode='same')(out) # out.shape=(self.uscData.mini_batch_size,1,8)
   out=keras.layers.Convolution1D(64, 3,activation='relu', border_mode='same')(out) # out.shape=(self.uscData.mini_batch_size,1,8)
   out=keras.layers.Convolution1D(64, 3,activation='relu', border_mode='same')(out) # out.shape=(self.uscData.mini_batch_size,1,8)
   out=keras.layers.Convolution1D(64, 3,activation='relu', border_mode='same')(out)
   out=keras.layers.Convolution1D(64, 3,activation='relu', border_mode='same')(out)
   out=keras.layers.Convolution1D(64, 3,activation='relu', border_mode='same')(out)
   out=keras.layers.Convolution1D(64, 3,activation='relu', border_mode='same')(out)
   out=keras.layers.Convolution1D(64, 3,activation='relu', border_mode='same')(out)
