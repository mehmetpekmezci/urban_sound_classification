...
class USCLogger :
 ...
 def logStepEnd(self,prepareDataTimes,trainingTimes,trainingAccuracies,testTimes,testAccuracies,trainingIterationNo):
    ...
    ## GRAPH (FOR LOGGING)
    tariningAcuracySummary = keras.backend.get_session().run(self.tfSummaryAccuracyMergedWriter, {self.tf_summary_accuracy_log_var: np.mean(trainingAccuracies)})
    self.trainingAccuracyWriter.add_summary(tariningAcuracySummary, trainingIterationNo)
    self.trainingAccuracyWriter.flush()

    testAcuracySummary = keras.backend.get_session().run(self.tfSummaryAccuracyMergedWriter, {self.tf_summary_accuracy_log_var:np.mean(testAccuracies)})
    self.testAccuracyWriter.add_summary(testAcuracySummary, trainingIterationNo)
    self.testAccuracyWriter.flush()


 def logAutoEncoderStepStart(self,trainingIterationNo):
    self.logger.info("##############################################################")
    self.logger.info("AutoEncoder Training Iteration : "+str(trainingIterationNo))

 def logAutoEncoderStepEnd(self,prepareDataTimes,trainingTimes,trainingLosses,trainingIterationNo):
