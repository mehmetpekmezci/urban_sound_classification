...
class NeuralNetworkModel :
 def __init__(self, session, logger, 
   ...
   self.real_y_values_metric_y=tf.placeholder(tf.float32, shape=(self.mini_batch_size, 1), name="real_y_values_metric_y")
   self.real_x_values_metric_x=tf.placeholder(tf.float32, shape=(self.mini_batch_size, 1), name="real_x_values_metric_x")
   ...
   with tf.name_scope('fourier_CNN_1'):
    for fourierCNNLayerNo in range(len(self.fourier_cnn_layers)) :
     ...
     previous_level_kernel_count_1=cnnKernelCount
   ...
   with tf.name_scope('fourier_CNN_2'):
    for fourierCNNLayerNo in range(len(self.fourier_cnn_layers)) :
     ...
     previous_level_kernel_count_2=cnnKernelCount
     fourierCNNOutput_2=previous_level_convolution_output_2

   ##
   ## CNN LAYERS 1
   ##
    previous_level_convolution_output_1=fourierCNNOutput_1
    for cnnLayerNo in range(len(self.cnn_kernel_counts)) :
     self.logger.info("fourierCNNOutput_2.shape="+str(previous_level_convolution_output_1.shape))
     ...
     cnnOutputChannel= cnnKernelCount   
     if cnnLayerNo == 0 :
       cnnInputChannel = int (previous_level_convolution_output_1.shape[3])
     else :
     ...
     if cnnPoolSizeY != 1 :
      ...
      if previous_level_kernel_count_1==cnnKernelCount :
       with tf.name_scope(cnnLayerName+"-residual"):
         previous_level_convolution_output_1=H+previous_level_convolution_output_1
         ## put the output of this layer to the next layer's input layer.
         self.logger.info(cnnLayerName+"_previous_level_convolution_output_residual.shape="+str(previous_level_convolution_output_1.shape))
      else :
         ## put the output of this layer to the next layer's input layer.
         previous_level_convolution_output_1=H

     previous_level_kernel_count_1=cnnKernelCount
     cnn_last_layer_output_1=previous_level_convolution_output_1
   
   ##
   ## CNN LAYERS 2
   ##
    previous_level_convolution_output_2=fourierCNNOutput_2
    for cnnLayerNo in range(len(self.cnn_kernel_counts)) :
     self.logger.info("fourierCNNOutput_2.shape="+str(previous_level_convolution_output_2.shape))
     cnnLayerName    = "cnn-"+str(cnnLayerNo)     
     cnnKernelCount  = self.cnn_kernel_counts[cnnLayerNo]   # cnnKernelCount tane cnnKernelSizeX * cnnKernelSizeY lik convolution kernel uygulanacak , sonucta 64x1x88200 luk tensor cikacak.
     cnnKernelSizeX  = self.cnn_kernel_x_sizes[cnnLayerNo]
     cnnKernelSizeY  = self.cnn_kernel_y_sizes[cnnLayerNo]         
     ...
     cnnOutputChannel= cnnKernelCount   
     if cnnLayerNo == 0 :
       cnnInputChannel = int (previous_level_convolution_output_2.shape[3])
     else :
       cnnInputChannel = self.cnn_kernel_counts[int(cnnLayerNo-1)]   
     ...
     if cnnPoolSizeY != 1 :
      ...
      if previous_level_kernel_count_2==cnnKernelCount :
       with tf.name_scope(cnnLayerName+"-residual"):
         previous_level_convolution_output_2=H+previous_level_convolution_output_2
         ## put the output of this layer to the next layer's input layer.
         self.logger.info(cnnLayerName+"_previous_level_convolution_output_residual.shape="+str(previous_level_convolution_output_2.shape))
      else :
         ## put the output of this layer to the next layer's input layer.
         previous_level_convolution_output_2=H

     previous_level_kernel_count_2=cnnKernelCount
     cnn_last_layer_output_2=previous_level_convolution_output_2
   ...
   with tf.name_scope('cnn_to_fc_reshape'):
    cnn_last_layer_output_flat_1 = tf.reshape( cnn_last_layer_output_1, [-1, int(cnn_last_layer_output_1.shape[1]*cnn_last_layer_output_1.shape[2]*cnn_last_layer_output_1.shape[3])] )
    self.logger.info("cnn_last_layer_output_flat1="+str( cnn_last_layer_output_flat_1))
    cnn_last_layer_output_flat_2 = tf.reshape( cnn_last_layer_output_2, [-1, int(cnn_last_layer_output_2.shape[1]*cnn_last_layer_output_2.shape[2]*cnn_last_layer_output_2.shape[3])] )
    self.logger.info("cnn_last_layer_output_flat2="+str( cnn_last_layer_output_flat_2))
    fourierCNNOutput_flat_1 = tf.reshape( fourierCNNOutput_1, [-1, int(fourierCNNOutput_1.shape[1]*fourierCNNOutput_1.shape[2]*fourierCNNOutput_1.shape[3])] )
    self.logger.info("fourierCNNOutput_flat_1="+str( fourierCNNOutput_flat_1))
    fourierCNNOutput_flat_2 = tf.reshape( fourierCNNOutput_2, [-1, int(fourierCNNOutput_2.shape[1]*fourierCNNOutput_2.shape[2]*fourierCNNOutput_2.shape[3])] )
    self.logger.info("fourierCNNOutput_flat_2="+str( fourierCNNOutput_flat_2))
   ...
   # Metric fully connected layer a ver
   ...
   #metric_y output=0/1  yes or no, meaning that these two outputs are the same or not
   metric_y_output_size=1
   # Map the NUMBER_OF_FULLY_CONNECTED_NEURONS features to OUTPUT_SIZE=NUMBER_OF_CLASSES(10) classes, one for each class
   with tf.name_scope('last_fc'):
    ...
    #h_fc2 =tf.nn.relu( tf.matmul(h_fc1_drop, W_fc2) + b_fc2)
    self.y_outputs_metric_y =tf.matmul(last_layer_output, W_fc2) + b_fc2
    self.logger.info("self.y_outputs_metric_y.shape="+str(self.y_outputs_metric_y.shape))
    

   # first_fouriercnn_output ve second_fouriercnn_output birlestir
   # Metric fully connected layer a ver
   classifier_outputs=tf.concat((fourierCNNOutput_flat_1,fourierCNNOutput_flat_2),1)
   last_layer_output=classifier_outputs

   ## METRIC LEARNING FOR X
   for fcLayerNo in range(len(self.fully_connected_layers)) :
   ...
   ##
    with tf.name_scope('calculate_loss'):
     ...
     cross_entropy_metric_y = tf.nn.softmax_cross_entropy_with_logits(labels=self.real_y_values_metric_y,logits=self.y_outputs_metric_y)
     self.loss_metric_y = tf.reduce_mean(cross_entropy_metric_y)
     cross_entropy_metric_x = tf.nn.softmax_cross_entropy_with_logits(labels=self.real_x_values_metric_x,logits=self.x_outputs_metric_x)
     self.loss_metric_x = tf.reduce_mean(cross_entropy_metric_x)
  ...
  ## gercekte ayni oluğ olmadigina gore duzeltme verilir.
   ...
   with tf.name_scope('optimizer'):
    ...
    self.optimizer_metric_y = tf.train.AdamOptimizer(learning_rate=self.learning_rate,beta1=self.learning_rate_beta1,beta2=self.learning_rate_beta2).minimize(self.loss_metric_y)
    self.optimizer_metric_x = tf.train.AdamOptimizer(learning_rate=self.learning_rate,beta1=self.learning_rate_beta1,beta2=self.learning_rate_beta2).minimize(self.loss_metric_x)
 ...
 def prepareData(self,data,augment):
  ...
  non_augmented_x=x_data
  if augment==True :
    x_data=augment_random(x_data)
  y_data=data[:,4*SOUND_RECORD_SAMPLING_RATE]
  y_data_one_hot_encoded=one_hot_encode_array(y_data)
  return non_augmented_x,x_data,y_data_one_hot_encoded

 def train(self,data1,data2):
  augment=True
  prepareDataTimeStart = int(round(time.time())) 
  non_augmented_x_1,x_data1,y_data1=self.prepareData(data1,augment)
  non_augmented_x_2,x_data2,y_data2=self.prepareData(data2,augment)
  prepareDataTimeStop = int(round(time.time())) 
  prepareDataTime=prepareDataTimeStop-prepareDataTimeStart
  trainingTimeStart = int(round(time.time())) 


  x_values_metric_x=np.zeros((self.mini_batch_size,1)) # [0,0,0,0,0]
  for i in range(self.mini_batch_size) :
    if  np.array_equal(non_augmented_x_1[i], non_augmented_x_2[i]) :
          x_values_metric_x[i][0]=1

  y_values_metric_y=np.zeros((self.mini_batch_size,1)) # [0,0,0,0,0]
  for i in range(self.mini_batch_size) :
    if  np.array_equal(y_data1[i], y_data2[i]) :
          y_values_metric_y[i][0]=1

  self.optimizer_1.run(feed_dict={self.x_input_1: x_data1, self.real_y_values_1:y_data1,self.x_input_2: x_data2, self.real_y_values_2:y_data2,self.real_y_values_metric_y:y_values_metric_y,self.keep_prob:self.keep_prob_constant})
  self.optimizer_2.run(feed_dict={self.x_input_1: x_data1, self.real_y_values_1:y_data1,self.x_input_2: x_data2, self.real_y_values_2:y_data2,self.real_y_values_metric_y:y_values_metric_y,self.keep_prob:self.keep_prob_constant})
  self.optimizer_metric_y.run(feed_dict={self.x_input_1: x_data1, self.real_y_values_1:y_data1,self.x_input_2: x_data2, self.real_y_values_2:y_data2,self.real_y_values_metric_y:y_values_metric_y,self.keep_prob:self.keep_prob_constant})
  self.optimizer_metric_x.run(feed_dict={self.x_input_1: x_data1, self.real_y_values_1:y_data1,self.x_input_2: x_data2, self.real_y_values_2:y_data2,self.real_x_values_metric_x:x_values_metric_x,self.keep_prob:self.keep_prob_constant})
