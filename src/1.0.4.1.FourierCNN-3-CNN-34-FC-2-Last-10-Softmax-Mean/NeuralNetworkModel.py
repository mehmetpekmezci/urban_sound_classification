...
class NeuralNetworkModel :
 def __init__(self, session, logger, 
   ...
   ##
   ## DEFINE PREVIOUS LOSS
   ##
   self.previous_loss=tf.placeholder(tf.float32, shape=(self.mini_batch_size, self.output_size), name="previous_loss")
   ...
   with tf.name_scope('calculate_loss'):
     ...
     self.logger.info("self.loss.shape="+str(self.loss.shape))
   
   ##
   ## SET OPTIMIZER
   ##
   with tf.name_scope('optimizer'):
    self.optimizer = tf.train.AdamOptimizer(learning_rate=self.learning_rate,beta1=self.learning_rate_beta1,beta2=self.learning_rate_beta2).minimize((self.loss+self.previous_loss)/2)
