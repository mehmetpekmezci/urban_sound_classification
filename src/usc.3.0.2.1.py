...
PARALLEL_CONVOLUTION_KERNELS_SIZES=np.array([ [7*5,5,7,3,3,2,3,3,2,3,2,1,2,1,2,1]  ])
...
def weight_variable_2d(shape):
  ...
  initial=np.zeros([conv_kernel_length_x,conv_kernel_length_y])

  for i in range(conv_kernel_length_x):
     initial[i]=truncated_normal_generator.rvs(conv_kernel_length_y)

  ## bir 0 bir 1
  for i in range(conv_kernel_length_x):
    for j in range(conv_kernel_length_y):
       if (j+i)%2==0 :
           initial[i,j]=0 
  print(initial)
