     ...
     if cnnPoolSizeY != 1 :
      ...
      if previous_level_kernel_count==cnnKernelCount :
       with tf.name_scope(cnnLayerName+"-residual"):
         this_layer_output=H+previous_level_convolution_output
         ## put the output of this layer to the next layer's input layer.
         self.logger.info(cnnLayerName+"_this_layer_convolution_output_residual.shape="+str(this_layer_output.shape))
      else :
         ## put the output of this layer to the next layer's input layer.
         this_layer_output=H

     with tf.name_scope('cnn-'+str(cnnLayerName)+'_batch_normlalization'):
       batch_mean, batch_var = tf.nn.moments(this_layer_output,[0,1,2])
       scale = tf.Variable(tf.ones(this_layer_output.shape[1]))
       beta = tf.Variable(tf.zeros(this_layer_output.shape[1]))
       batch_normalized_this_layer_output = tf.nn.batch_normalization(this_layer_output,batch_mean,batch_var,beta,scale,epsilon)
       self.logger.info("batch_normalization_cnn-"+str(cnnLayerName)+".shape="+str(batch_normalized_this_layer_output.shape))

     previous_level_convolution_output=batch_normalized_this_layer_output
