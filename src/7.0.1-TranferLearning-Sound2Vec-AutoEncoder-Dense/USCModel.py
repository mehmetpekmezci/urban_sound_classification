...
class USCModel :
 ...
 def prepareData(self,data,augment):
  ...
  encodedValue=encodedValue.reshape(encodedValue.shape[0],encodedValue.shape[1]*encodedValue.shape[2])
 ...
 def buildModel(self):
   ...
   out=keras.layers.Dense(units = 2048,activation='relu')(layer_input)
   out=keras.layers.BatchNormalization()(out)
