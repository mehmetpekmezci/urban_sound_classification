 ...
 def __init__(self, session, logger, 
   ...
   with tf.name_scope('fourier_CNN'):
    for fourierCNNLayerNo in range(len(self.fourier_cnn_layers)) :
     ...
     with tf.name_scope('fc-'+str(cnnLayerName)+'_batch_normlalization'):    
       batch_mean, batch_var = tf.nn.moments(H,[0,1,2])
       scale = tf.Variable(tf.ones(H.shape[1]))
       beta = tf.Variable(tf.zeros(H.shape[1]))
       batch_normalized_H = tf.nn.batch_normalization(H,batch_mean,batch_var,beta,scale,epsilon)
       self.logger.info("batch_normalization_fourier_cnn-"+str(cnnLayerName)+".shape="+str(batch_normalized_H.shape))
    
     if cnnPoolSizeY != 1 :
      with tf.name_scope(cnnLayerName+"-pool"):
       P = tf.nn.max_pool(batch_normalized_H, ksize=[1, cnnPoolSizeX,cnnPoolSizeY, 1],strides=[1, cnnPoolSizeX,cnnPoolSizeY , 1], padding='SAME') 
       ...
       previous_level_convolution_output=batch_normalized_H
