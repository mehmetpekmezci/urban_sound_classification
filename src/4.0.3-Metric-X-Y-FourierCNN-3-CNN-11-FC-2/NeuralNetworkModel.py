...
class NeuralNetworkModel :
 def __init__(self, session, logger, 
   ...
   self.real_x_values_adverserial=tf.placeholder(tf.float32, shape=(self.mini_batch_size, 1), name="real_x_values_adverserial")
   ...
   with tf.name_scope('cnn_to_fc_reshape'):
    ...
    self.logger.info("cnn_last_layer_output_flat="+str( cnn_last_layer_output_flat_1))
    ...
    self.logger.info("cnn_last_layer_output_flat="+str( cnn_last_layer_output_flat_2))
 ...
 def prepareData(self,data,augment):
  ...
  non_augmented_x_data=x_data
  if augment==True :
    x_data=augment_random(x_data)
  y_data=data[:,4*SOUND_RECORD_SAMPLING_RATE]
  y_data_one_hot_encoded=one_hot_encode_array(y_data)
  return x_data,y_data_one_hot_encoded,non_augmented_x_data

 def train(self,data1,data2):
  augment=True
  prepareDataTimeStart = int(round(time.time())) 
  x_data1,y_data1,non_augmented_x_data_1=self.prepareData(data1,augment)
  x_data2,y_data2,non_augmented_x_data_2=self.prepareData(data2,augment)
  ...
  x_values_adverserial=np.zeros((self.mini_batch_size,1)) # [0,0,0,0,0]
  for i in range(self.mini_batch_size) :
    if  np.array_equal(non_augmented_x_data_1[i], non_augmented_x_data_2[i]) :
          x_values_adverserial[i][0]=1
  ...
  self.optimizer_adverserial.run(feed_dict={self.x_input_1: x_data1, self.real_y_values_1:y_data1,self.x_input_2: x_data2, self.real_y_values_2:y_data2,self.real_y_values_adverserial:y_values_adverserial,self.real_x_values_adverserial:x_values_adverserial,sself.keep_prob:self.keep_prob_constant})
