...
class NeuralNetworkModel :
 def __init__(self, session, logger, input_size_x=INPUT_SIZE_X, input_size_y=INPUT_SIZE_Y, output_size=OUTPUT_SIZE , cnn_kernel_counts=CNN_KERNEL_COUNTS, cnn_kernel_x_sizes=CNN_KERNEL_X_SIZES, 
              ...
              learning_rate=LEARNING_RATE, mini_batch_size=MINI_BATCH_SIZE, number_of_time_slices=NUMBER_OF_MERGED_TRACK,number_of_lstm_layers=NUMBER_OF_LSTM_LAYERS, lstm_state_size=LSTM_STATE_SIZE
...
#  logger.info("prepare data started = "+str(t1))
  ...
  #   logger.info("getting augmented data finished = "+str(tx2))
     ...
     x_data_sliced_shifted[:,:,int(j*SLIDE_STEP):int(j*SLIDE_STEP+TRACK_LENGTH),:]=x_data_sliced[:,:,:,:]
