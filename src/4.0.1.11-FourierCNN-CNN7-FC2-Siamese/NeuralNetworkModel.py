 ...
 def __init__(self, session, logger, 
   ...
   with tf.name_scope('fourier_CNN'):
    for fourierCNNLayerNo in range(len(self.fourier_cnn_kernel_counts)) :
     ...
     cnnKernelSizeX  = self.cnn_kernel_x_sizes[fourierCNNLayerNo]
     cnnKernelSizeY  = self.cnn_kernel_y_sizes[fourierCNNLayerNo]         
     cnnStrideSizeX  = self.cnn_stride_x_sizes[fourierCNNLayerNo] 
     cnnStrideSizeY  = self.cnn_stride_y_sizes[fourierCNNLayerNo]                     
     cnnPoolSizeX    = self.cnn_pool_x_sizes[fourierCNNLayerNo]          
     cnnPoolSizeY    = self.cnn_pool_y_sizes[fourierCNNLayerNo]      
...
#     self.loss_single=LOSS_WEIGHT_1*self.loss_1+LOSS_WEIGHT_2*self.loss_2+LOSS_WEIGHT_3*self.loss_adverserial
     self.loss_single=LOSS_WEIGHT_1*self.loss_1+LOSS_WEIGHT_2*self.loss_2
