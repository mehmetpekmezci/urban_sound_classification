        ...
        for fold in np.random.permutation(fold_dirs):
          if fold == "fold10":
            ...
            for current_batch_counter in range(int(current_fold_data.shape[0]/MINI_BATCH_SIZE)) :
              ...
              test_predicted_y, test_accuracy = lstm_net.run_step(test_x_data)
            ...
            for current_batch_counter in range(int(current_fold_data.shape[0]/MINI_BATCH_SIZE)) :
              ...
              train_x_data_sliced=slice_data(train_x_data,NUMBER_OF_SLICES)
              train_y_data=train_data[:,4*SOUND_RECORD_SAMPLING_RATE]
              train_y_data_one_hot_encoded=one_hot_encode_array(train_y_data)
              train_y_data_one_hot_encoded_replicated=replicate_data(train_y_data_one_hot_encoded,NUMBER_OF_SLICES)  
              train_accuracy = lstm_net.train_batch(train_x_data_sliced, train_y_data_one_hot_encoded_replicated)
