...
def generate_single_noise(MAX_NUMBER_OF_NOISE_FREQUENCY_PER_SAMPLE,SOUND_RECORD_SAMPLING_RATE,DURATION,MAX_HEARING_FREQUENCY):
    generated_data=np.zeros(DURATION*SOUND_RECORD_SAMPLING_RATE,np.float32)
    randomValue=np.random.rand()
    number_of_frequencies=int(randomValue*MAX_NUMBER_OF_NOISE_FREQUENCY_PER_SAMPLE)
...
def generate_normalized_synthetic_noise(numberOfSamples):
    global MAX_NUMBER_OF_NOISE_FREQUENCY_PER_SAMPLE,SOUND_RECORD_SAMPLING_RATE,DURATION,MAX_HEARING_FREQUENCY
    samples=np.zeros((numberOfSamples,DURATION*SOUND_RECORD_SAMPLING_RATE),np.float32)
    for i in range(numberOfSamples):
       samples[i]=generate_single_noise(MAX_NUMBER_OF_NOISE_FREQUENCY_PER_SAMPLE,SOUND_RECORD_SAMPLING_RATE,DURATION,MAX_HEARING_FREQUENCY)
