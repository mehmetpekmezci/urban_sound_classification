...
class NeuralNetworkModel :
 def __init__(self, session, logger, 
   ...
   self.x_input                      = tf.placeholder(tf.float32, shape=(self.mini_batch_size, self.input_size_x,self.input_size_y), name="input")
   with tf.name_scope('input_reshape'):
    print("self.x_input.shape="+str(self.x_input.shape))
    self.x_input_reshaped = tf.reshape(self.x_input, [self.mini_batch_size, self.input_size_x, self.input_size_y, number_of_input_channels])
