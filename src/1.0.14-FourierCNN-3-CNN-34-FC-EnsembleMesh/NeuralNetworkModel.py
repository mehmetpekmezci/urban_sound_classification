...
class NeuralNetworkModel :
 def __init__(self, session, logger, 
              ...
              numberOfEnsemblesPerMeshLayer=NUMBER_OF_ENSEMBLES_PER_MESH_LAYER,
              numberOfEnsemblesMesh=NUMBER_OF_ENSEMBLES_MESH,
              numberOfFCCellPerEnsembleUnit=NUMBER_OF_FC_CELL_PER_ENSEMBLE_UNIT,
   ...
   self.numberOfEnsemblesPerMeshLayer=numberOfEnsemblesPerMeshLayer
   self.numberOfEnsemblesMesh = numberOfEnsemblesMesh
   self.numberOfFCCellPerEnsembleUnit = numberOfFCCellPerEnsembleUnit
   ...
   previousLayerFcEnsembleOutputs=[]
   for ensembleNo in range(self.numberOfEnsemblesMesh+1) : # this +1 is very important, it is used for the loops last element's next element
   ...
   for ensembleLayerNo in range(self.numberOfEnsemblesPerMeshLayer) :
    for ensembleNo in range(self.numberOfEnsemblesMesh) :
     with tf.name_scope('ensemble-layer-L'+str(ensembleLayerNo)+"_ensemble-no-N"+str(ensembleNo)):
      ...
      B_fc1 = tf.Variable(tf.constant(0.1, shape=[self.numberOfFCCellPerEnsembleUnit]))
      self.logger.info("B_L"+str(ensembleLayerNo)+"N"+str(ensembleNo)+".shape="+str(B_fc1.shape))
      matmul_fc1=tf.matmul(tf.concat((previousLayerFcEnsembleOutputs[ensembleNo],previousLayerFcEnsembleOutputs[int(ensembleNo+1)]),1), W_fc1)+B_fc1

     with tf.name_scope('ensemble-'+str(ensembleNo)+'_batch_normlalization'):
      batch_mean, batch_var = tf.nn.moments(matmul_fc1,[0])
      scale = tf.Variable(tf.ones(self.numberOfFCCellPerEnsembleUnit))
      beta = tf.Variable(tf.zeros(self.numberOfFCCellPerEnsembleUnit))
      batch_normalization_fc1 = tf.nn.batch_normalization(matmul_fc1,batch_mean,batch_var,beta,scale,epsilon)
      self.logger.info("batch_normalization_ensemble-"+str(ensembleNo)+".shape="+str(batch_normalization_fc1.shape))

     with tf.name_scope('ensemble-'+str(ensembleNo)+'_batch_normalized_relu'):
      h_fc1 = tf.nn.relu( batch_normalization_fc1 )
      self.logger.info("h_ensemble-"+str(ensembleNo)+".shape="+str(h_fc1.shape))

     # Dropout - controls the complexity of the model, prevents co-adaptation of features.
     with tf.name_scope('ensemble-'+str(ensembleNo)+'_dropout'):
      h_fc1_drop = tf.nn.dropout(h_fc1, self.keep_prob)
      self.logger.info("h_fc-"+str(ensembleNo)+"_drop.shape="+str(h_fc1_drop.shape))
      previousLayerFcEnsembleOutputs[ensembleNo]=h_fc1_drop ## pass the result of this layer to the next layer.

    # set first and last element with the same values for loop.
    previousLayerFcEnsembleOutputs[-1]=previousLayerFcEnsembleOutputs[0]


   last_layer_output=tf.concat(previousLayerFcEnsembleOutputs,1)
   self.logger.info("concatanated last_layer_output="+str(last_layer_output))
   ...
   with tf.name_scope('last_fc'):
    W_fc2 =  tf.Variable( tf.truncated_normal([int(last_layer_output.shape[1]), self.output_size], stddev=0.1))
