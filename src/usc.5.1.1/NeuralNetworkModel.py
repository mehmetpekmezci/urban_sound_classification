...
class NeuralNetworkModel :
 ...
 def prepareData(self,data,augment):
  ...
  ## slice x_data into number_of_time_slices, then for each slice apply gammatone conversion (this will take the time silced sound data as input, and returns the values from each filterbank , so the output will of size 
  ## (minibatch_size, number_of_time_slices,number_of_gammatone_filters)

  x_data_gammatone=slice_and_convert_to_gammatone(np.asarray(x_data),self.mini_batch_size,self.number_of_time_slices,self.input_size,SOUND_RECORD_SAMPLING_RATE,GAMMATONE_NUMBER_OF_FILTERS,GAMMATONE_WINDOW_TIME,GAMMATONE_HOP_TIME)
...
#       x_data_gammatone[miniBatch,timeSlice]=get_gammatone_specgram(x_data_reshaped[miniBatch,timeSlice],SOUND_RECORD_SAMPLING_RATE,GAMMATONE_NUMBER_OF_FILTERS,GAMMATONE_WINDOW_TIME,GAMMATONE_HOP_TIME)
#
