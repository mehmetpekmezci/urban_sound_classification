...
class Data :
 ...
 def __init__(self,logger,SCRIPT_DIR):
   ...
   self.MAIN_DATA_DIR = SCRIPT_DIR+'/../../data/'
   self.RAW_DATA_DIR = MAIN_DATA_DIR+'/0.raw/UrbanSound8K/audio'
   self.CSV_DATA_DIR=MAIN_DATA_DIR+"/1.csv"
   self.NP_DATA_DIR=MAIN_DATA_DIR+"/2.np"
 ...
 def prepareData(self):
    ...
    if not os.path.exists(MAIN_DATA_DIR+"/0.raw/UrbanSound8K"):
      if os.path.exists(MAIN_DATA_DIR+"/0.raw/UrbanSound8K.tar.gz"):
         self.logger.info("Extracting "+MAIN_DATA_DIR+"/0.raw/UrbanSound8K.tar.gz")
         tar = tarfile.open(MAIN_DATA_DIR+"/0.raw/UrbanSound8K.tar.gz")
         tar.extractall(MAIN_DATA_DIR+'/../data/0.raw')
         tar.close()
         self.logger.info("Extracted "+MAIN_DATA_DIR+"/0.raw/UrbanSound8K.tar.gz")
      else :   
         self.logger.info("download "+MAIN_DATA_DIR+"/0.raw/UrbanSound8K.tar.gz from http://serv.cusp.nyu.edu/files/jsalamon/datasets/content_loader.php?id=2  using firefox browser or chromium  and re-run this script")
         exit(1)
 ...
 def load_all_np_data_back_to_memory(self,fold_data_dictionary):
   ...
   for fold in FOLD_DIRS:
       self.logger.info ("loading from "+MAIN_DATA_DIR+"/2.np/"+fold+".npy  ...")
 ...
 def generate_single_synthetic_sample(self,MAX_NUMBER_OF_SYNTHETIC_FREQUENCY_PER_SAMPLE,SOUND_RECORD_SAMPLING_RATE,DURATION,MAX_HEARING_FREQUENCY):
 ...
 def generate_normalized_synthetic_samples(self,fold):
    ...
    if fold not in GENERATED_DATA :
      if os.path.exists(MAIN_DATA_DIR+"/2.np/generated_data-"+fold+".npy"):
        self.logger.info("Loading Already Generated Synthetic Sound Sample Data from "+MAIN_DATA_DIR+"/2.np/generated_data-"+fold+".npy")
        ...
        for i in range(NUMBER_OF_SYNTHETIC_TRAINNG_SAMPLES):
          samples[i]=self.generate_single_synthetic_sample(MAX_NUMBER_OF_SYNTHETIC_FREQUENCY_PER_SAMPLE,SOUND_RECORD_SAMPLING_RATE,DURATION,MAX_HEARING_FREQUENCY)
        ...
        self.logger.info("Saving Generated Synthetic Sound Sample Data to "+MAIN_DATA_DIR+"/2.np/generated_data-"+fold+".npy")
