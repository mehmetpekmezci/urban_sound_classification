...
class NeuralNetworkModel :
 def __init__(self, session, logger, 
              ...
              residual_attention_cnn_kernel_counts=RESIDUAL_ATTENTION_CNN_KERNEL_COUNTS,residual_attention_cnn_kernel_encoders=RESIDUAL_ATTENTION_CNN_KERNEL_ENCODERS,
              residual_attention_cnn_kernel_x_sizes=FOURIER_CNN_KERNEL_X_SIZES,residual_attention_cnn_kernel_y_sizes=FOURIER_CNN_KERNEL_Y_SIZES,
              residual_attention_cnn_stride_x_sizes=FOURIER_CNN_STRIDE_X_SIZES,residual_attention_cnn_stride_y_sizes=FOURIER_CNN_STRIDE_Y_SIZES,
              residual_attention_cnn_pool_x_sizes=FOURIER_CNN_POOL_X_SIZES,residual_attention_cnn_pool_y_sizes=FOURIER_CNN_POOL_Y_SIZES, 
     ENCODERS         
...
RESIDUAL_ATTENTION_CNN_POOL_Y_SIZES        = np.array([ 16, 1,16])
   ...
   ##
   ## RESIDUAL ATTENTION  CNN LAYERS
   ##
   with tf.name_scope('residual_attention_CNN'):
    for residualAttentionCNNLayerNo in range(len(self.residual_attention_cnn_kernel_counts)) :
     self.logger.info("previous_level_convolution_output.shape="+str(previous_level_convolution_output.shape))
     cnnLayerName    = "res-att-cnn-"+str(residualAttentionCNNLayerNo)          
     cnnKernelCount  = self.residual_attention_cnn_kernel_counts[residualAttentionCNNLayerNo]
     cnnKernelSizeX  = self.residual_attention_cnn_kernel_x_sizes[residualAttentionCNNLayerNo]
     cnnKernelSizeY  = self.residual_attention_cnn_kernel_y_sizes[residualAttentionCNNLayerNo]         
     cnnStrideSizeX  = self.residual_attention_cnn_stride_x_sizes[residualAttentionCNNLayerNo] 
     cnnStrideSizeY  = self.residual_attention_cnn_stride_y_sizes[residualAttentionCNNLayerNo]                     
     cnnPoolSizeX    = self.residual_attention_cnn_pool_x_sizes[residualAttentionCNNLayerNo]          
     cnnPoolSizeY    = self.residual_attention_cnn_pool_y_sizes[residualAttentionCNNLayerNo]      
     ...
     if fourierCNNLayerNo == 0 :
       ...
       cnnInputChannel = self.residual_attention_cnn_kernel_counts[int(residualAttentionCNNLayerNo-1)]   
   ...
   number_of_fully_connected_layer_neurons=self.fully_connected_layers[-1]
