...
def overlapping_hanning_slice(x_data,y_data):
    global MINI_BATCH_SIZE,TRACK_LENGTH,TIME_SLICE_LENGTH,TIME_SLICE_OVERLAP_LENGTH,NUMBER_OF_TIME_SLICES

    sliced_and_overlapped_data=np.zeros([MINI_BATCH_SIZE,NUMBER_OF_TIME_SLICES,TIME_SLICE_LENGTH])
    sliced_and_overlapped_Y_data=np.zeros([MINI_BATCH_SIZE,NUMBER_OF_TIME_SLICES,1])
    ...
    for i in range(MINI_BATCH_SIZE):
        for j in range(NUMBER_OF_TIME_SLICES):
            step_index=j*step
            sliced_and_overlapped_Y_data[i][j]=y_data[i]
    ...
    sliced_and_overlapped_data=np.reshape(sliced_and_overlapped_data,[MINI_BATCH_SIZE*NUMBER_OF_TIME_SLICES,TIME_SLICE_LENGTH])
    sliced_and_overlapped_Y_data=np.reshape(sliced_and_overlapped_Y_data,[MINI_BATCH_SIZE*NUMBER_OF_TIME_SLICES,1])
    return sliced_and_overlapped_data,sliced_and_overlapped_Y_data
