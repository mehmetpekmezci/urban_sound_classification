...
class NeuralNetworkModel :
 def __init__(self, session, logger, 
   ...
   with tf.name_scope('input_reshape'):
     self.x_input_reshaped = tf.reshape(last_layer_output, [self.mini_batch_size, self.input_size_y, int(last_layer_output.shape[1]), number_of_input_channels])
   ...
   for fcLayerNo in range(len(self.fully_connected_layers)) :
    ...
    if fcLayerNo == 0 :
        W_fc1 =  tf.Variable( tf.truncated_normal([int(last_layer_output.shape[1]), number_of_fully_connected_layer_neurons], stddev=0.1))
        self.logger.info("W_fc-double--"+str(fcLayerNo)+".shape="+str(W_fc1.shape))
        B_fc1 = tf.Variable(tf.constant(0.1, shape=[number_of_fully_connected_layer_neurons]))
        self.logger.info("B_fc-double--"+str(fcLayerNo)+".shape="+str(B_fc1.shape))
        matmul_fc1=tf.matmul(last_layer_output, W_fc1)+B_fc1
        self.logger.info("matmul_fc-double--"+str(fcLayerNo)+".shape="+str(matmul_fc1.shape))
        batch_mean, batch_var = tf.nn.moments(matmul_fc1,[0])
        scale = tf.Variable(tf.ones(number_of_fully_connected_layer_neurons))
        beta = tf.Variable(tf.zeros(number_of_fully_connected_layer_neurons))
        batch_normalization_fc1 = tf.nn.batch_normalization(matmul_fc1,batch_mean,batch_var,beta,scale,epsilon)
        self.logger.info("batch_normalization_fc-double--"+str(fcLayerNo)+".shape="+str(batch_normalization_fc1.shape))
        h_fc1 = tf.nn.sigmoid( batch_normalization_fc1 )
        self.logger.info("h_fc-double--"+str(fcLayerNo)+".shape="+str(h_fc1.shape))
        h_fc1_drop = tf.nn.dropout(h_fc1, self.keep_prob)
        h_fc1_drop=tf.concat((first_fc_output,h_fc1_drop),1)
        self.logger.info("h_fc-double--"+str(fcLayerNo)+"_drop.shape="+str(h_fc1_drop.shape))
        last_layer_output=h_fc1_drop
        first_fc_output=last_layer_output
   ...
   ##
    with tf.name_scope('calculate_loss'):
     self.cross_entropy = tf.nn.softmax_cross_entropy_with_logits(labels=self.real_y_values,logits=self.y_outputs)
 ...
 def train(self,data):
  ...
  loss = self.loss.eval(feed_dict={self.x_input: x_data, self.real_y_values:y_data,self.keep_prob: 1.0})
  trainingTimeStop = int(round(time.time())) 
  trainingTime=trainingTimeStop-trainingTimeStart
  return trainingTime,trainingAccuracy,loss,prepareDataTime
     
 def test(self,data):
  testTimeStart = int(round(time.time())) 
  augment=False
  x_data,y_data=self.prepareData(data,augment) 
  y_outputs = self.y_outputs.eval(feed_dict={self.x_input: x_data, self.real_y_values:y_data,self.keep_prob: 1.0})
